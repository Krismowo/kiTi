...
class KiSprite extends Bitmap 
{
	...
	public function setAnimFrames(animName:String, animFrames:Array<KiFrame>, fps:Int = 0){
		...
		for (i in animFrames){
			var frame:KiFrame = i;
			var expFrame:BitmapData;
			expFrame = new BitmapData(Std.int(frame.width), Std.int(frame.height),true);
			for (y in 0...Std.int(frame.height + 1)){
				for (x in 0...Std.int(frame.width + 1)){
					var sheetpixel = sheet.getPixel32(Std.int(frame.x + x), Std.int(frame.y + y));
					//if()
					expFrame.setPixel32(x, y, sheetpixel);
				}
			}
	...
	public function loadJsonArray(jsonData:String, bitmapData:BitmapData){
		...
		var cookedJson:KiJsonAnimInfo = Json.parse(jsonData);
		...
		if (curAnim != null){
			if (curAnim.playing){
				...
				if(bitmapData != curAnim.frames[curAnim.curframe]){
					bitmapData = curAnim.frames[curAnim.curframe];
				}
			}
		}
	}
